...
namespace RzWeb
{
    public class PurchaseLine : Line
    {
        ...
        public PurchaseLine(ContextRz context, orddet_line line)
            : base(context, line)
        {
            xActions.TheSetup = new ActSetupOrder(Rz5.Enums.OrderType.Purchase);
            TheContext = context;
            lblOrderNumber = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderNumber", "", "Purchase " + line.ordernumber_purchase + " Line Item")));
            string name = "";
            if (line.OrderObjectGet(context, Rz5.Enums.OrderType.Purchase) != null)
            ...
            lblUnPacked = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblUnPacked", "Received", Tools.Number.LongFormat(line.quantity_unpacked))));
            txtMFG = (TextControl)SpotAdd(ControlAdd(new TextControl("manufacturer", "Manufacturer", line.manufacturer)));
            txtDC = (TextControl)SpotAdd(ControlAdd(new TextControl("datecode_purchase", ((ContextRz)context).DateCodeCaption, line.datecode_purchase)));
            ...
            txtCustPart = (TextControl)SpotAdd(ControlAdd(new TextControl("internal_vendor", "Internal Part", line.internal_vendor)));
            ArrayList a = new ArrayList();
            a.Add("Y");
            a.Add("N");
            a.Add("U");
            cboROHS = (ComboBoxControl)SpotAdd(ControlAdd(new ComboBoxControl("rohs_info", "RoHS Info", line.rohs_info, a)));
            cboShipVia = (RzWeb.ChoicesControl)SpotAdd(ControlAdd(new RzWeb.ChoicesControl("shipvia_purchase", "Ship Via", line.shipvia_purchase, GetChoiceList(context, "shipvia"), "", "shipvia")));
            cboShipAccount = (ComboBoxControl)SpotAdd(ControlAdd(new ComboBoxControl("shippingaccount_purchase", "Shipping Account", line.shippingaccount_purchase, GetShippingAccounts(context))));
            txtTracking = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("tracking_purchase", "Tracking Numbers", line.tracking_purchase)));
            ...
            dtReceiveDue = (DateControl)SpotAdd(ControlAdd(new DateControl("receive_date_due", "Receive Date Due", line.receive_date_due)));
            dtReceiveActual = (DateControl)SpotAdd(ControlAdd(new DateControl("receive_date_actual", "Receive Date Actual", line.receive_date_actual)));
            xPack = (PackControl)SpotAdd(ControlAdd(new PackControl("xPack", "", context, this, line.PacksInVar, line, false, false, Rz5.Enums.OrderType.Purchase)));
            xPack.PackRefreshed += new EventHandler(xPack_PackRefreshed);
            AdjustControls();
        }
        public override String Title(Context x)
        {
            string s = "Purchase Line";
            if (TheLine != null)
                s = "Purchase " + TheLine.ordernumber_purchase + " Line";
            return s;
        }
        public override void RenderContents(Context x, StringBuilder sb, Screen screenHandle, ViewHandle viewHandle, System.Web.SessionState.HttpSessionState session, System.Web.UI.Page page)
        {
            ...
            sb.AppendLine("<div id=\"viewpurchase_" + Uid + "\" style=\"position: absolute; top: 0px;\">");
            //tabs
        ...
        protected override void ResizeRender(System.Text.StringBuilder sb, System.Web.UI.Page page)
        {
            ...
            sb.AppendLine("$('#viewpurchase_" + Uid + "').css('top', $('#rz_menu').height() + $('#rz_menu').position().top + 10);");
            sb.AppendLine("$('#viewpurchase_" + Uid + "').css('height', $(window).height() - $('#viewpurchase_" + Uid + "').position().top - 70);");
            sb.AppendLine("$('#viewpurchase_" + Uid + "').css('width', " + xActions.Select + ".position().left - $('#viewpurchase_" + Uid + "').position().left - 15);");
            sb.AppendLine("$('#tabHeader_" + Uid + "').css('left', 5);");
            sb.AppendLine("$('#tabHeader_" + Uid + "').css('top', 0);");
            sb.AppendLine("$('#tabHeader_" + Uid + "').css('width', 600);");
            sb.AppendLine("$('#tabHeader_" + Uid + "').css('height', 400);");
            ...
            sb.AppendLine(txtUnitCost.PlaceRight(cboCondition));
        ...
        {
            TheLine = orddet_line.GetById(TheContext, TheLine.unique_id);
            TheLine.Update(TheContext);
            lblUnPacked.ValueSet(TheLine.quantity_unpacked);
            lblUnPacked.Change();
        }
        private void ShowOrder(ContextRz x)
        {
            RzWeb.Purchase q = new RzWeb.Purchase((ContextRz)x, (ordhed_purchase)TheLine.OrderObjectGet(x, Rz5.Enums.OrderType.Purchase));
