...
namespace RzWeb
{
    public class RMA : _Item
    {
        public ordhed_rma TheRMA
        {
            get
            {
                return (ordhed_rma)Item;
            }
        }
        ...
        public RMA(ContextRz x, ordhed_rma i)
            : base(x, i)
        {
            TheRMA.CalculateAllAmounts(x);
            TheRMA.trackingnumber = TheRMA.TrackingNumbersGet(x);
            lblInvoice = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblInvoice", "", TheRMA.FriendlyOrderType)));
            lblOrderNumber = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderNumber", "", TheRMA.ordernumber)));
            ...
            lblSubTotal = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblSubTotal", "", x.TheSys.CurrencySymbol + Tools.Number.MoneyFormat(TheRMA.sub_total))));
            lblShipping = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblShipping", "", "Extra:")));
            txtShipping = (MoneyControl)SpotAdd(ControlAdd(new MoneyControl("shippingamount", "", TheRMA.shippingamount, x.TheSys.CurrencySymbol)));
            lblOrderTotalCap = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderTotalCap", "", "Total:")));
            lblOrderTotal = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderTotal", "", x.TheSys.CurrencySymbol + Tools.Number.MoneyFormat(TheRMA.ordertotal))));
            lblOrderDate = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderDate", "", "Order Date")));
            txtOrderDate = (LabelControl)SpotAdd(ControlAdd(new LabelControl("txtOrderDate", "", TheRMA.orderdate.ToString())));
            ...
            agentSalesAgent = (AgentControl)SpotAdd(ControlAdd(new AgentControl("base_mc_user_uid|agentname", "Sales Agent", TheRMA.base_mc_user_uid, TheRMA.agentname, "base_mc_user_uid", "agentname", agents)));
            txtPhone = (TextControl)SpotAdd(ControlAdd(new TextControl("primaryphone", "Phone", TheRMA.primaryphone)));
            txtFax = (TextControl)SpotAdd(ControlAdd(new TextControl("primaryfax", "Fax", TheRMA.primaryfax)));
            txtEmail = (TextControl)SpotAdd(ControlAdd(new TextControl("primaryemailaddress", "Email", TheRMA.primaryemailaddress)));
            txtInternalComment = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("internalcomment", "Internal Comment", TheRMA.internalcomment)));
            txtPrintComment = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("printcomment", "Print Comment", TheRMA.printcomment)));
            xCompany = (CompanyContactControl)SpotAdd(ControlAdd(new CompanyContactControl("base_company_uid|companyname|base_companycontact_uid|contactname", "Customer", TheRMA.base_company_uid, TheRMA.companyname, "base_company_uid", "companyname", TheRMA.base_companycontact_uid, TheRMA.contactname, "base_companycontact_uid", "contactname")));
            ...
            txtReference = (TextControl)SpotAdd(ControlAdd(new TextControl("orderreference", "PO Number", TheRMA.orderreference)));
            cboTerms = (RzWeb.ChoicesControl)SpotAdd(ControlAdd(new RzWeb.ChoicesControl("terms", "Terms", TheRMA.terms, GetChoiceList(x, "terms"), "", "terms")));
            cboShipVia = (RzWeb.ChoicesControl)SpotAdd(ControlAdd(new RzWeb.ChoicesControl("shipvia", "Ship Via", TheRMA.shipvia, GetChoiceList(x, "shipvia"), "", "shipvia")));
            //Address Info
            txtBillingAdr = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("billingaddress", "Billing Address", TheRMA.billingaddress)));
            txtShippingAdr = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("shippingaddress", "Shipping Address", TheRMA.shippingaddress)));
            txtTracking = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("trackingnumber", "Tracking Numbers", TheRMA.trackingnumber)));
            txtTracking.DisableEdit = true;
            cboShippingAccount = (RzWeb.ChoicesControl)SpotAdd(ControlAdd(new RzWeb.ChoicesControl("shippingaccount", "Shipping Account", TheRMA.shippingaccount, LoadAccountList(x), "", "null")));
            ...
            lvDetails.TheArgs = TheRMA.DetailArgsGet(Context);
            lvDetails.TheArgs.LiveItems = TheRMA.DetailsVar.RefsGetAsItems(Context);
        ...
        public override void RenderContents(Context x, StringBuilder sb, Screen screenHandle, ViewHandle viewHandle, System.Web.SessionState.HttpSessionState session, System.Web.UI.Page page)
        {
            ...
            sb.AppendLine("        </div>");
            s = " visibility: hidden; ";
            int pa = TheRMA.DetailsListPutAwayable((ContextRz)x).Count;
            if (pa > 0)
                s = "";
        ...
        {
            TheRMA.CalculateAllAmounts(x);
            lblSubTotal.ValueSet(x.TheSys.CurrencySymbol + Tools.Number.MoneyFormat(TheRMA.SubTotal(x)));
            lblOrderTotal.ValueSet(x.TheSys.CurrencySymbol + Tools.Number.MoneyFormat(TheRMA.ordertotal));
            lblSubTotal.Change();
        ...
        public override void Act(Context x, SpotActArgs args)
        {
            ...
            switch (args.ActionId)
            {
                ...
                case "action1":
                    ...
                    break;
                case "change_date":
                    ChangeOrderDate((ContextRz)x, TheRMA.orderdate.ToString());
                    break;
        ...
        private void AdjustControls()
        {
            ...
            else if (TheRMA.isclosed)
                lblStatus.TextForeColor = Color.Blue;
            else if (TheRMA.onhold)
        ...
        {
            TheRMA.DateChange(x);
            txtOrderDate.ValueSet(TheRMA.orderdate.ToString());
            txtOrderDate.Change();
        }
        private void ChangeOrderNumber(ContextRz x)
        {
            TheRMA.NumberChange(x);
            lblOrderNumber.ValueSet(TheRMA.ordernumber);
            Change();
        }
        ...
        {
            ArrayList a = new ArrayList();
            Rz5.company c = (Rz5.company)TheRMA.CompanyVar.RefGet(x);
            if (c == null)
                return a;
            ...
            switch (val.Trim().ToLower())
            {
                ...
                default:
                    if (val.StartsWith("Contact address for"))
                    {
                        ...
                        if (!Tools.Strings.StrCmp(contactname, TheRMA.ContactVar.RefGet(x).contactname))
                        {
                            x.TheLeader.Tell("This doesn't appear to be the same contact that's assigned to this order.");
                            return;
                        }
                        txtBillingAdr.Value = TheRMA.ContactVar.RefGet(x).BuildAddress();
                    }
                    ...
                    {
                        companyaddress a = companyaddress.GetByDescription(x, TheRMA.base_company_uid, val);
                        if (a != null)
            ...
            switch (val.Trim().ToLower())
            {
                ...
                default:
                    if (val.StartsWith("Contact address for"))
                    {
                        ...
                        txtShippingAdr.Value = TheRMA.ContactVar.RefGet(x).BuildAddress();
                    }
                    else if (val.ToLower().StartsWith("address option"))
                    {
        ...
        protected override void SaveData(Context x, SpotActArgs args, Dictionary<string, string> values)
        {
            ...
            if (!Tools.Strings.StrExt(s))
                return;
            foreach (orddet_line l in TheRMA.DetailsList((ContextRz)x))
            {
                if (!Tools.Strings.StrExt(l.shipvia_rma))
                {
                    l.shipvia_rma = s;
                    l.Update(x);
                }
        ...
        private void xCompany_CompanyChanged(ContextNM x, company c, ViewHandle v)
        {
            if (c == null)
                return;
            TheRMA.CompanyVar.RefSet(x, c);
            TheRMA.AbsorbCompany((ContextRz)x, c);
            TheRMA.Update(x);
            LoadAccountList((ContextRz)x);
            cboTerms.ValueSet(TheRMA.terms);
        ...
        private void xCompany_ContactChanged(ContextNM x, companycontact c, ViewHandle v)
        {
            ...
            txtFax.Change();
            txtEmail.Change();
            TheRMA.ContactVar.RefSet(x, c);
            TheRMA.Update(x);
        }
        private void lvDetails_AddNewItem(Context x, System.Web.UI.Page page, ViewHandle viewHandle)
        {
            TheRMA.DetailAddWithChecks((ContextRz)x);
            TheRMA.Update((ContextRz)x);
        ...
        private void lvDetails_ItemDoubleClicked(Context x, IItem item, System.Web.UI.Page page, ViewHandle viewHandle)
        {
            ...
            if (d == null)
                return;
            RzWeb.RMALine q = new RzWeb.RMALine((ContextRz)x, d);
