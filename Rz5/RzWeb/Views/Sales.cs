...
namespace RzWeb
{
    public class Sales : _Item
    {
        public ordhed_sales TheSale
        {
            get
            {
                return (ordhed_sales)Item;
            }
        }
        ...
        public Sales(ContextRz x, ordhed_sales i)
            : base(x, i)
        {
            TheSale.CalculateAllAmounts(x);
            TheSale.trackingnumber = TheSale.TrackingNumbersGet(x);
            lblInvoice = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblInvoice", "", TheSale.FriendlyOrderType)));
            lblOrderNumber = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderNumber", "", TheSale.ordernumber)));
            string status1 = GetStatus(x);
            lblStatus = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblStatus", "", status1)));
            ...
            lblSubTotal = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblSubTotal", "", x.TheSys.CurrencySymbol + Tools.Number.MoneyFormat(TheSale.sub_total))));
            lblShipping = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblShipping", "", "Extra:")));
            txtShipping = (MoneyControl)SpotAdd(ControlAdd(new MoneyControl("shippingamount", "", TheSale.shippingamount, x.TheSys.CurrencySymbol)));
            lblOrderTotalCap = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderTotalCap", "", "Total:")));
            lblOrderTotal = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderTotal", "", x.TheSys.CurrencySymbol + Tools.Number.MoneyFormat(TheSale.ordertotal))));
            lblOrderDate = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderDate", "", "Order Date")));
            txtOrderDate = (LabelControl)SpotAdd(ControlAdd(new LabelControl("txtOrderDate", "", TheSale.orderdate.ToString())));
            ...
            agentSalesAgent = (AgentControl)SpotAdd(ControlAdd(new AgentControl("base_mc_user_uid|agentname", "Sales Agent", TheSale.base_mc_user_uid, TheSale.agentname, "base_mc_user_uid", "agentname", agents)));
            txtPhone = (TextControl)SpotAdd(ControlAdd(new TextControl("primaryphone", "Phone", TheSale.primaryphone)));
            txtFax = (TextControl)SpotAdd(ControlAdd(new TextControl("primaryfax", "Fax", TheSale.primaryfax)));
            txtEmail = (TextControl)SpotAdd(ControlAdd(new TextControl("primaryemailaddress", "Email", TheSale.primaryemailaddress)));
            txtInternalComment = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("internalcomment", "Internal Comment", TheSale.internalcomment)));
            txtPrintComment = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("printcomment", "Print Comment", TheSale.printcomment)));
            xCompany = (CompanyContactControl)SpotAdd(ControlAdd(new CompanyContactControl("base_company_uid|companyname|base_companycontact_uid|contactname", "Customer", TheSale.base_company_uid, TheSale.companyname, "base_company_uid", "companyname", TheSale.base_companycontact_uid, TheSale.contactname, "base_companycontact_uid", "contactname")));
            ...
            txtReference = (TextControl)SpotAdd(ControlAdd(new TextControl("orderreference", "PO Number", TheSale.orderreference)));
            cboTerms = (RzWeb.ChoicesControl)SpotAdd(ControlAdd(new RzWeb.ChoicesControl("terms", "Terms", TheSale.terms, GetChoiceList(x, "terms"), "", "terms")));
            cboShipVia = (RzWeb.ChoicesControl)SpotAdd(ControlAdd(new RzWeb.ChoicesControl("shipvia", "Ship Via", TheSale.shipvia, GetChoiceList(x, "shipvia"), "", "shipvia")));
            //Address Info
            txtBillingAdr = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("billingaddress", "Billing Address", TheSale.billingaddress)));
            txtShippingAdr = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("shippingaddress", "Shipping Address", TheSale.shippingaddress)));
            txtTracking = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("trackingnumber", "Tracking Numbers", TheSale.trackingnumber)));
            txtTracking.DisableEdit = true;
            txtStatus = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("txtStatus", "Actions Needed To Complete", "")));
            cboShippingAccount = (RzWeb.ChoicesControl)SpotAdd(ControlAdd(new RzWeb.ChoicesControl("shippingaccount", "Shipping Account", TheSale.shippingaccount, LoadAccountList(x), "", "null")));
            ...
            lvDetails.TheArgs = TheSale.DetailArgsGet(Context);
            lvDetails.TheArgs.LiveItems = TheSale.DetailsVar.RefsGetAsItems(Context);
        ...
        private string SetValues(ContextRz x)
        {
            StringBuilder sb = new StringBuilder();
            string status = "";
            int count = 0;
            bool so_verifiy = TheSale.CheckVerify((ContextRz)x, sb);
            int ready = TheSale.DetailsListCompleteReady((ContextRz)x, ref status, ref count).Count;
            if (Tools.Strings.StrExt(status))
                sb.AppendLine(status);
            txtStatus.Value = sb.ToString();
            return sb.ToString();
        }
        ...
        {
            string status = SetValues((ContextRz)x);
            TheView = viewHandle;
            TheSession = session;
            ThePage = page;
            string s = "";
            ...
            sb.AppendLine("        </div>");
            s = "";
            bool so_verifiy = TheSale.CheckVerify((ContextRz)x, new StringBuilder());
            int invoice_able = TheSale.DetailsListInvoiceable((ContextRz)x).Count;
            if (invoice_able > 0 && so_verifiy)
                s = " visibility: hidden; ";
            sb.AppendLine("        <div id=\"action1_" + Uid + "\" class=\"jqborderstyle ui-corner_all rz-margin\" style=\"position: absolute; padding: 4px; top: 0px; height: 60px; width: 185px; " + s + "\">");
            sb.AppendLine("<input id=\"cmdAction1\" type=\"button\" value=\"Sales Order Active\" onclick=\"" + ActionScript("'action1'") + "\">");
            Buttonize(viewHandle, "cmdAction1", "so_active.png");
            sb.AppendLine("        </div>");
            s = " visibility: hidden; ";
            if (invoice_able > 0 && so_verifiy)
                s = "";
            sb.AppendLine("        <div id=\"action2_" + Uid + "\" class=\"jqborderstyle ui-corner_all rz-margin\" style=\"position: absolute; padding: 4px; top: 74px; height: 60px; width: 155px; " + s + "\">");
            sb.AppendLine("<input id=\"cmdAction2\" type=\"button\" value=\"Start Shipment\" onclick=\"" + ActionScript("'action2'") + "\">");
            Buttonize(viewHandle, "cmdAction2", "start_shipment.png");
            sb.AppendLine("        </div>");
            s = "";
            ...
            string img = "";
            if (Tools.Strings.StrExt(status))
                img = "<img id=\"img_status_" + Uid + "\" alt=\"img_status\" src=\"" + page.ResolveClientUrl("~/Graphics/caution.png") + "\"/>";
            ...
            sb.AppendLine("                <li><a href=\"#tabStatus\" style=\"font-size: xx-small\">" + img + "&nbsp;Status</a></li>");
            sb.AppendLine("            </ul>");
            ...
            sb.AppendLine("            </div>");
            sb.AppendLine("            <div style=\"position: relative;\" id=\"tabStatus\">");
            txtStatus.Render(x, sb, screenHandle, viewHandle, session, page);
            sb.AppendLine("            </div>");
            sb.AppendLine("        </div>");
        ...
        protected override void ResizeRender(System.Text.StringBuilder sb, System.Web.UI.Page page)
        {
            ...
            sb.AppendLine("$('#action1_" + Uid + "').css('left', $('#totals_box_" + Uid + "').position().left + 23);");
            sb.AppendLine("$('#action2_" + Uid + "').css('left', $('#totals_box_" + Uid + "').position().left + 38);");
            ...
            sb.AppendLine(txtReference.PlaceBelow(txtPhone));
            sb.AppendLine(txtReference.PlaceRight(txtEmail));
            sb.AppendLine(cboTerms.PlaceBelow(txtEmail));
            sb.AppendLine(cboShippingAccount.PlaceBelow(txtEmail));
            sb.AppendLine(cboShippingAccount.PlaceRight(cboTerms));
            sb.AppendLine(cboShipVia.PlaceBelow(txtEmail));
            sb.AppendLine(cboShipVia.PlaceRight(cboShippingAccount));
        ...
        {
            TheSale.CalculateAllAmounts(x);
            lblSubTotal.ValueSet(x.TheSys.CurrencySymbol + Tools.Number.MoneyFormat(TheSale.SubTotal(x)));
            lblOrderTotal.ValueSet(x.TheSys.CurrencySymbol + Tools.Number.MoneyFormat(TheSale.ordertotal));
            lblSubTotal.Change();
        ...
        public override void Act(Context x, SpotActArgs args)
        {
            ...
            switch (args.ActionId)
            {
                ...
                case "action2":
                    ...
                    break;
                case "change_date":
                    ChangeOrderDate((ContextRz)x, TheSale.orderdate.ToString());
                    break;
        ...
        private void AdjustControls()
        {
            ...
            else if (TheSale.isclosed)
                lblStatus.TextForeColor = Color.Blue;
            else if (TheSale.onhold)
            ...
            txtStatus.CaptionFontSize = FontSize.XSmall;
            txtStatus.TextFontSize = FontSize.XSmall;
            txtStatus.FixedWidth = 345;
            txtStatus.Rows = 12;
            ...
            cboShippingAccount.FixedWidth = 113;
        ...
        {
            TheSale.DateChange(x);
            txtOrderDate.ValueSet(TheSale.orderdate.ToString());
            txtOrderDate.Change();
        }
        private void ChangeOrderNumber(ContextRz x)
        {
            TheSale.NumberChange(x);
            lblOrderNumber.ValueSet(TheSale.ordernumber);
            Change();
        }
        ...
        {
            ArrayList a = new ArrayList();
            Rz5.company c = (Rz5.company)TheSale.CompanyVar.RefGet(x);
            if (c == null)
                return a;
            ...
            switch (val.Trim().ToLower())
            {
                ...
                default:
                    if (val.StartsWith("Contact address for"))
                    {
                        ...
                        if (!Tools.Strings.StrCmp(contactname, TheSale.ContactVar.RefGet(x).contactname))
                        {
                            x.TheLeader.Tell("This doesn't appear to be the same contact that's assigned to this order.");
                            return;
                        }
                        txtBillingAdr.Value = TheSale.ContactVar.RefGet(x).BuildAddress();
                    }
                    ...
                    {
                        companyaddress a = companyaddress.GetByDescription(x, TheSale.base_company_uid, val);
                        if (a != null)
            ...
            switch (val.Trim().ToLower())
            {
                ...
                default:
                    if (val.StartsWith("Contact address for"))
                    {
                        ...
                        txtShippingAdr.Value = TheSale.ContactVar.RefGet(x).BuildAddress();
                    }
                    else if (val.ToLower().StartsWith("address option"))
                    {
        ...
        private void Action1(ContextRz x)
        {
            ...
            bool so_verifiy = TheSale.CheckVerify((ContextRz)x, new StringBuilder());
            int ready = TheSale.DetailsListCompleteReady((ContextRz)x, ref status, ref count).Count;
            if (ready <= 0 && !so_verifiy)
            {
                x.TheLeader.Tell("This order is not ready to be completed. You can check what still needs to be done under the 'Status' tab.");
                return;
            }
            ContextRz xx = (ContextRz)x.Clone();
            String cid = xx.TheDelta.StartChangeCache();
            SalesOrderCompleteResult r = TheSale.CompleteSalesOrder(xx);
        ...
        private void Action2(ContextRz x)
        {
            ContextRz xx = (ContextRz)x.Clone();
            String cid = xx.TheDelta.StartChangeCache();
            List<ordhed_invoice> Invoices = TheSale.MakeInvoiceWithChecks(xx);
            xx.TheDelta.EndChangeCache(x, cid);
            foreach (ordhed_invoice i in Invoices)
            {
                if (i != null)
                    x.Show(new ShowArgsOrder(x, i, Rz5.Enums.OrderType.Invoice));
            }
        }
        private void MakeLink(ContextRz x)
        {
            OrderLinkArgs args = new OrderLinkArgs((ordhed_new)TheSale);
            args.TheLinkType = Rz5.Enums.OrderType.Purchase;
            args = ((LeaderWebUserRz)x.TheLeaderRz).OrderLinkChoose(x, args, "Search for the PO below:", "", TheScreen, TheView, TheSession, ThePage);
            ...
            foreach (OrderLinkLine l in args.Lines)
            {
                ...
                TheSale.Details.RefsAdd(x, l.TheLine);
                l.TheLine.ParentOrderTypeSet(x, Rz5.Enums.OrderType.Sales);
                x.Update(l.TheLine);
            }
        }
        protected override void SaveData(Context x, SpotActArgs args, Dictionary<string, string> values)
        {
            ...
            if (!Tools.Strings.StrExt(s))
                return;
            foreach (orddet_line l in TheSale.DetailsList((ContextRz)x))
            {
        ...
        private void xCompany_CompanyChanged(ContextNM x, company c, ViewHandle v)
        {
            if (c == null)
                return;
            TheSale.CompanyVar.RefSet(x, c);
            TheSale.AbsorbCompany((ContextRz)x, c);
            TheSale.Update(x);
            LoadAccountList((ContextRz)x);
            cboTerms.ValueSet(TheSale.terms);
            ...
            cboShippingAccount.ValueSet(TheSale.shippingaccount);
            cboTerms.Change();
            txtPhone.Change();
            txtFax.Change();
            txtEmail.Change();
        ...
        private void xCompany_ContactChanged(ContextNM x, companycontact c, ViewHandle v)
        {
            ...
            txtFax.Change();
            txtEmail.Change();
            TheSale.ContactVar.RefSet(x, c);
            TheSale.Update(x);
        }
        private void lvDetails_AddNewItem(Context x, System.Web.UI.Page page, ViewHandle viewHandle)
        {
            if (x.TheLeader.AskYesNo("Is the new line already on a purchase order?"))
            {
                MakeLink((ContextRz)x);
                lvDetails.TheArgs.LiveItems = TheSale.DetailsVar.RefsGetAsItems((ContextRz)x);
                lvDetails.RowSource = new RowSourceItem(lvDetails.TheArgs.LiveItems.AllGet(Context));
                lvDetails.Change();
                TheSale.Update((ContextRz)x);
                return;
            }
            TheSale.DetailAddWithChecks((ContextRz)x);
            TheSale.Update((ContextRz)x);
        ...
        private void lvDetails_ItemDoubleClicked(Context x, IItem item, System.Web.UI.Page page, ViewHandle viewHandle)
        {
            ...
            if (d == null)
                return;
            RzWeb.SalesLine q = new RzWeb.SalesLine((ContextRz)x, d);
