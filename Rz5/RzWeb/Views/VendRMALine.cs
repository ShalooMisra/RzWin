...
namespace RzWeb
{
    public class VendRMALine : Line
    {
        ...
        public VendRMALine(ContextRz context, orddet_line line)
            : base(context, line)
        {
            xActions.TheSetup = new ActSetupOrder(Rz5.Enums.OrderType.VendRMA);
            TheContext = context;
            lblOrderNumber = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderNumber", "", "VendRMA " + line.ordernumber_vendrma + " Line Item")));
            string name = "";
            if (line.OrderObjectGet(context, Rz5.Enums.OrderType.VendRMA) != null)
            ...
            lblPacked = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblPacked", "Packed", Tools.Number.LongFormat(line.quantity_packed_vendrma))));            
            txtUnitPrice = (MoneyControl)SpotAdd(ControlAdd(new MoneyControl("unit_price_vendrma", "Unit Price", line.unit_price_vendrma)));
            ...
            cboShipVia = (RzWeb.ChoicesControl)SpotAdd(ControlAdd(new RzWeb.ChoicesControl("shipvia_vendrma", "Ship Via", line.shipvia_vendrma, GetChoiceList(context, "shipvia"), "", "shipvia")));
            cboShipAccount = (ComboBoxControl)SpotAdd(ControlAdd(new ComboBoxControl("shippingaccount_vendrma", "Shipping Account", line.shippingaccount_vendrma, GetShippingAccounts(context))));
            txtTracking = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("tracking_vendrma", "Tracking Numbers", line.tracking_vendrma)));
            ...
            dtReceiveDue = (DateControl)SpotAdd(ControlAdd(new DateControl("ship_date_vendrma_due", "Ship Date Due", line.ship_date_vendrma_due)));
            dtReceiveActual = (DateControl)SpotAdd(ControlAdd(new DateControl("ship_date_vendrma_actual", "Ship Date Actual", line.ship_date_vendrma_actual)));
            xPack = (PackControl)SpotAdd(ControlAdd(new PackControl("xPack", "", context, this, line.PacksVendRMAVar, line, true, true, Rz5.Enums.OrderType.VendRMA)));
            xPack.PackRefreshed += new EventHandler(xPack_PackRefreshed);
            AdjustControls();
        }
        public override String Title(Context x)
        {
            string s = "Vendor RMA Line";
            if (TheLine != null)
                s = "Vendor RMA " + TheLine.ordernumber_vendrma + " Line";
            return s;
        }
        public override void RenderContents(Context x, StringBuilder sb, Screen screenHandle, ViewHandle viewHandle, System.Web.SessionState.HttpSessionState session, System.Web.UI.Page page)
        {
            ...
            sb.AppendLine("<div id=\"viewvendrma_" + Uid + "\" style=\"position: absolute; top: 0px;\">");
            //tabs
        ...
        protected override void ResizeRender(System.Text.StringBuilder sb, System.Web.UI.Page page)
        {
            ...
            sb.AppendLine("$('#viewvendrma_" + Uid + "').css('top', $('#rz_menu').height() + $('#rz_menu').position().top + 10);");
            sb.AppendLine("$('#viewvendrma_" + Uid + "').css('height', $(window).height() - $('#viewvendrma_" + Uid + "').position().top - 70);");
            sb.AppendLine("$('#viewvendrma_" + Uid + "').css('width', " + xActions.Select + ".position().left - $('#viewvendrma_" + Uid + "').position().left - 15);");
        ...
        private void xPack_PackRefreshed(object sender, EventArgs e)
        {
            ...
            lblPacked.ValueSet(TheLine.quantity_packed_vendrma);
            lblPacked.Change();
        }
        private void ShowOrder(ContextRz x)
        {
            RzWeb.VendRMA q = new RzWeb.VendRMA((ContextRz)x, (ordhed_vendrma)TheLine.OrderObjectGet(x, Rz5.Enums.OrderType.VendRMA));
