...
namespace RzWeb
{
    public class Purchase : _Item
    {
        public ordhed_purchase ThePurchase
        {
            get
            {
        ...
        public Purchase(ContextRz x, ordhed_purchase i)
            : base(x, i)
        {
            ThePurchase.CalculateAllAmounts(x);
            ThePurchase.trackingnumber = ThePurchase.TrackingNumbersGet(x);
            lblInvoice = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblInvoice", "", ThePurchase.FriendlyOrderType)));
            lblOrderNumber = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderNumber", "", ThePurchase.ordernumber)));
            ...
            lblSubTotal = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblSubTotal", "", x.TheSys.CurrencySymbol + Tools.Number.MoneyFormat(ThePurchase.sub_total))));
            lblShipping = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblShipping", "", "Extra:")));
            txtShipping = (MoneyControl)SpotAdd(ControlAdd(new MoneyControl("shippingamount", "", ThePurchase.shippingamount, x.TheSys.CurrencySymbol)));
            lblOrderTotalCap = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderTotalCap", "", "Total:")));
            lblOrderTotal = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderTotal", "", x.TheSys.CurrencySymbol + Tools.Number.MoneyFormat(ThePurchase.ordertotal))));
            lblOrderDate = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderDate", "", "Order Date")));
            txtOrderDate = (LabelControl)SpotAdd(ControlAdd(new LabelControl("txtOrderDate", "", ThePurchase.orderdate.ToString())));
            ...
            agentSalesAgent = (AgentControl)SpotAdd(ControlAdd(new AgentControl("base_mc_user_uid|agentname", "Sales Agent", ThePurchase.base_mc_user_uid, ThePurchase.agentname, "base_mc_user_uid", "agentname", agents)));
            txtPhone = (TextControl)SpotAdd(ControlAdd(new TextControl("primaryphone", "Phone", ThePurchase.primaryphone)));
            txtFax = (TextControl)SpotAdd(ControlAdd(new TextControl("primaryfax", "Fax", ThePurchase.primaryfax)));
            txtEmail = (TextControl)SpotAdd(ControlAdd(new TextControl("primaryemailaddress", "Email", ThePurchase.primaryemailaddress)));
            txtInternalComment = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("internalcomment", "Internal Comment", ThePurchase.internalcomment)));
            txtPrintComment = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("printcomment", "Print Comment", ThePurchase.printcomment)));
            xCompany = (CompanyContactControl)SpotAdd(ControlAdd(new CompanyContactControl("base_company_uid|companyname|base_companycontact_uid|contactname", "Customer", ThePurchase.base_company_uid, ThePurchase.companyname, "base_company_uid", "companyname", ThePurchase.base_companycontact_uid, ThePurchase.contactname, "base_companycontact_uid", "contactname")));
            ...
            txtReference = (TextControl)SpotAdd(ControlAdd(new TextControl("soreference", "SO Number", ThePurchase.soreference)));
            cboTerms = (RzWeb.ChoicesControl)SpotAdd(ControlAdd(new RzWeb.ChoicesControl("terms", "Terms", ThePurchase.terms, GetChoiceList(x, "terms"), "", "terms")));
            cboShipVia = (RzWeb.ChoicesControl)SpotAdd(ControlAdd(new RzWeb.ChoicesControl("shipvia", "Ship Via", ThePurchase.shipvia, GetChoiceList(x, "shipvia"), "", "shipvia")));
            //Address Info
            txtBillingAdr = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("billingaddress", "Billing Address", ThePurchase.billingaddress)));
            txtShippingAdr = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("shippingaddress", "Shipping Address", ThePurchase.shippingaddress)));
            txtTracking = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("trackingnumber", "Tracking Numbers", ThePurchase.trackingnumber)));
            txtTracking.DisableEdit = true;
            cboShippingAccount = (RzWeb.ChoicesControl)SpotAdd(ControlAdd(new RzWeb.ChoicesControl("shippingaccount", "Shipping Account", ThePurchase.shippingaccount, LoadAccountList(x), "", "null")));
            ...
            lvDetails.TheArgs = ThePurchase.DetailArgsGet(Context);
            lvDetails.TheArgs.LiveItems = ThePurchase.DetailsVar.RefsGetAsItems(Context);
        ...
        public override void RenderContents(Context x, StringBuilder sb, Screen screenHandle, ViewHandle viewHandle, System.Web.SessionState.HttpSessionState session, System.Web.UI.Page page)
        {
            ...
            sb.AppendLine("        </div>");
            s = " visibility: hidden; ";
            int pa = ThePurchase.DetailsListPutAwayable((ContextRz)x).Count;
            if (pa > 0)
                s = "";
            sb.AppendLine("        <div id=\"action1_" + Uid + "\" class=\"jqborderstyle ui-corner_all rz-margin\" style=\"position: absolute; padding: 4px; top: 0px; height: 60px; width: 103px; " + s + "\">");
            sb.AppendLine("<input id=\"cmdAction1\" type=\"button\" value=\"Put Away\" onclick=\"" + ActionScript("'action1'") + "\">");
            Buttonize(viewHandle, "cmdAction1", "put_away.png");
            sb.AppendLine("        </div>");
            s = "";
        ...
        protected override void ResizeRender(System.Text.StringBuilder sb, System.Web.UI.Page page)
        {
            ...
            sb.AppendLine("$('#action1_" + Uid + "').css('left', $('#totals_box_" + Uid + "').position().left + 63);");
        ...
        {
            ThePurchase.CalculateAllAmounts(x);
            lblSubTotal.ValueSet(x.TheSys.CurrencySymbol + Tools.Number.MoneyFormat(ThePurchase.SubTotal(x)));
            lblOrderTotal.ValueSet(x.TheSys.CurrencySymbol + Tools.Number.MoneyFormat(ThePurchase.ordertotal));
            lblSubTotal.Change();
        ...
        public override void Act(Context x, SpotActArgs args)
        {
            ...
            switch (args.ActionId)
            {
                ...
                case "action1":
                    ...
                    break;
                case "change_date":
                    ChangeOrderDate((ContextRz)x, ThePurchase.orderdate.ToString());
                    break;
        ...
        private void AdjustControls()
        {
            ...
            if (ThePurchase.isvoid)
                lblStatus.TextForeColor = Color.DarkGray;
            else if (ThePurchase.isclosed)
                lblStatus.TextForeColor = Color.Blue;
            else if (ThePurchase.onhold)
        ...
        {
            ThePurchase.DateChange(x);
            txtOrderDate.ValueSet(ThePurchase.orderdate.ToString());
            txtOrderDate.Change();
        }
        private void ChangeOrderNumber(ContextRz x)
        {
            ThePurchase.NumberChange(x);
            lblOrderNumber.ValueSet(ThePurchase.ordernumber);
            Change();
        }
        ...
        {
            ArrayList a = new ArrayList();
            Rz5.company c = (Rz5.company)ThePurchase.CompanyVar.RefGet(x);
            if (c == null)
                return a;
            ...
            switch (val.Trim().ToLower())
            {
                ...
                default:
                    if (val.StartsWith("Contact address for"))
                    {
                        ...
                        if (!Tools.Strings.StrCmp(contactname, ThePurchase.ContactVar.RefGet(x).contactname))
                        {
                            x.TheLeader.Tell("This doesn't appear to be the same contact that's assigned to this order.");
                            return;
                        }
                        txtBillingAdr.Value = ThePurchase.ContactVar.RefGet(x).BuildAddress();
                    }
                    ...
                    {
                        companyaddress a = companyaddress.GetByDescription(x, ThePurchase.base_company_uid, val);
                        if (a != null)
            ...
            switch (val.Trim().ToLower())
            {
                ...
                default:
                    if (val.StartsWith("Contact address for"))
                    {
                        ...
                        txtShippingAdr.Value = ThePurchase.ContactVar.RefGet(x).BuildAddress();
                    }
                    else if (val.ToLower().StartsWith("address option"))
                    {
        ...
        protected override void SaveData(Context x, SpotActArgs args, Dictionary<string, string> values)
        {
            ...
            if (!Tools.Strings.StrExt(s))
                return;
            foreach (orddet_line l in ThePurchase.DetailsList((ContextRz)x))
            {
                if (!Tools.Strings.StrExt(l.shipvia_purchase))
                {
                    l.shipvia_purchase = s;
                    l.Update(x);
                }
            }
        }
        private void xCompany_CompanyChanged(ContextNM x, company c, ViewHandle v)
        {
            if (c == null)
                return;
            ThePurchase.CompanyVar.RefSet(x, c);
            ThePurchase.AbsorbCompany((ContextRz)x, c);
            ThePurchase.Update(x);
            LoadAccountList((ContextRz)x);
            cboTerms.ValueSet(ThePurchase.terms);
        ...
        private void xCompany_ContactChanged(ContextNM x, companycontact c, ViewHandle v)
        {
            ...
            txtFax.Change();
            txtEmail.Change();
            ThePurchase.ContactVar.RefSet(x, c);
            ThePurchase.Update(x);
        }
        private void lvDetails_AddNewItem(Context x, System.Web.UI.Page page, ViewHandle viewHandle)
        {
            ThePurchase.DetailAddWithChecks((ContextRz)x);
            ThePurchase.Update((ContextRz)x);
        ...
        private void lvDetails_ItemDoubleClicked(Context x, IItem item, System.Web.UI.Page page, ViewHandle viewHandle)
        {
            ...
            if (d == null)
                return;
            RzWeb.PurchaseLine q = new RzWeb.PurchaseLine((ContextRz)x, d);
