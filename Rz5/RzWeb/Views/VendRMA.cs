...
namespace RzWeb
{
    public class VendRMA : _Item
    {
        public ordhed_vendrma TheVendRMA
        {
            get
            {
                return (ordhed_vendrma)Item;
            }
        }
        ...
        public VendRMA(ContextRz x, ordhed_vendrma i)
            : base(x, i)
        {
            TheVendRMA.CalculateAllAmounts(x);
            TheVendRMA.trackingnumber = TheVendRMA.TrackingNumbersGet(x);
            lblInvoice = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblInvoice", "", TheVendRMA.FriendlyOrderType)));
            lblOrderNumber = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderNumber", "", TheVendRMA.ordernumber)));
            ...
            lblSubTotal = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblSubTotal", "", x.TheSys.CurrencySymbol + Tools.Number.MoneyFormat(TheVendRMA.sub_total))));
            lblShipping = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblShipping", "", "Extra:")));
            txtShipping = (MoneyControl)SpotAdd(ControlAdd(new MoneyControl("shippingamount", "", TheVendRMA.shippingamount, x.TheSys.CurrencySymbol)));
            lblOrderTotalCap = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderTotalCap", "", "Total:")));
            lblOrderTotal = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderTotal", "", x.TheSys.CurrencySymbol + Tools.Number.MoneyFormat(TheVendRMA.ordertotal))));
            lblOrderDate = (LabelControl)SpotAdd(ControlAdd(new LabelControl("lblOrderDate", "", "Order Date")));
            txtOrderDate = (LabelControl)SpotAdd(ControlAdd(new LabelControl("txtOrderDate", "", TheVendRMA.orderdate.ToString())));
            ...
            agentSalesAgent = (AgentControl)SpotAdd(ControlAdd(new AgentControl("base_mc_user_uid|agentname", "Sales Agent", TheVendRMA.base_mc_user_uid, TheVendRMA.agentname, "base_mc_user_uid", "agentname", agents)));
            txtPhone = (TextControl)SpotAdd(ControlAdd(new TextControl("primaryphone", "Phone", TheVendRMA.primaryphone)));
            txtFax = (TextControl)SpotAdd(ControlAdd(new TextControl("primaryfax", "Fax", TheVendRMA.primaryfax)));
            txtEmail = (TextControl)SpotAdd(ControlAdd(new TextControl("primaryemailaddress", "Email", TheVendRMA.primaryemailaddress)));
            txtInternalComment = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("internalcomment", "Internal Comment", TheVendRMA.internalcomment)));
            txtPrintComment = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("printcomment", "Print Comment", TheVendRMA.printcomment)));
            xCompany = (CompanyContactControl)SpotAdd(ControlAdd(new CompanyContactControl("base_company_uid|companyname|base_companycontact_uid|contactname", "Customer", TheVendRMA.base_company_uid, TheVendRMA.companyname, "base_company_uid", "companyname", TheVendRMA.base_companycontact_uid, TheVendRMA.contactname, "base_companycontact_uid", "contactname")));
            ...
            txtReference = (TextControl)SpotAdd(ControlAdd(new TextControl("orderreference", "PO Number", TheVendRMA.orderreference)));
            cboTerms = (RzWeb.ChoicesControl)SpotAdd(ControlAdd(new RzWeb.ChoicesControl("terms", "Terms", TheVendRMA.terms, GetChoiceList(x, "terms"), "", "terms")));
            cboShipVia = (RzWeb.ChoicesControl)SpotAdd(ControlAdd(new RzWeb.ChoicesControl("shipvia", "Ship Via", TheVendRMA.shipvia, GetChoiceList(x, "shipvia"), "", "shipvia")));
            //Address Info
            txtBillingAdr = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("billingaddress", "Billing Address", TheVendRMA.billingaddress)));
            txtShippingAdr = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("shippingaddress", "Shipping Address", TheVendRMA.shippingaddress)));
            txtTracking = (TextAreaControl)SpotAdd(ControlAdd(new TextAreaControl("trackingnumber", "Tracking Numbers", TheVendRMA.trackingnumber)));
            txtTracking.DisableEdit = true;
            cboShippingAccount = (RzWeb.ChoicesControl)SpotAdd(ControlAdd(new RzWeb.ChoicesControl("shippingaccount", "Shipping Account", TheVendRMA.shippingaccount, LoadAccountList(x), "", "null")));
            ...
            lvDetails.TheArgs = TheVendRMA.DetailArgsGet(Context);
            lvDetails.TheArgs.LiveItems = TheVendRMA.DetailsVar.RefsGetAsItems(Context);
        ...
        public override void RenderContents(Context x, StringBuilder sb, Screen screenHandle, ViewHandle viewHandle, System.Web.SessionState.HttpSessionState session, System.Web.UI.Page page)
        {
            ...
            sb.AppendLine("        </div>");
            s = " visibility: hidden; ";
            int pa = TheVendRMA.DetailsListShippable((ContextRz)x).Count;
            if (pa > 0)
                s = "";            
        ...
        {
            TheVendRMA.CalculateAllAmounts(x);
            lblSubTotal.ValueSet(x.TheSys.CurrencySymbol + Tools.Number.MoneyFormat(TheVendRMA.SubTotal(x)));
            lblOrderTotal.ValueSet(x.TheSys.CurrencySymbol + Tools.Number.MoneyFormat(TheVendRMA.ordertotal));
            lblSubTotal.Change();
        ...
        public override void Act(Context x, SpotActArgs args)
        {
            ...
            switch (args.ActionId)
            {
                ...
                case "action1":
                    ...
                    break;
                case "change_date":
                    ChangeOrderDate((ContextRz)x, TheVendRMA.orderdate.ToString());
                    break;
        ...
        private void AdjustControls()
        {
            ...
            if (TheVendRMA.isvoid)
                lblStatus.TextForeColor = Color.DarkGray;
            else if (TheVendRMA.isclosed)
                lblStatus.TextForeColor = Color.Blue;
            else if (TheVendRMA.onhold)
        ...
        {
            TheVendRMA.DateChange(x);
            txtOrderDate.ValueSet(TheVendRMA.orderdate.ToString());
            txtOrderDate.Change();
        }
        private void ChangeOrderNumber(ContextRz x)
        {
            TheVendRMA.NumberChange(x);
            lblOrderNumber.ValueSet(TheVendRMA.ordernumber);
            Change();
        }
        ...
        {
            ArrayList a = new ArrayList();
            Rz5.company c = (Rz5.company)TheVendRMA.CompanyVar.RefGet(x);
            if (c == null)
                return a;
            ...
            switch (val.Trim().ToLower())
            {
                ...
                default:
                    if (val.StartsWith("Contact address for"))
                    {
                        ...
                        if (!Tools.Strings.StrCmp(contactname, TheVendRMA.ContactVar.RefGet(x).contactname))
                        {
                            x.TheLeader.Tell("This doesn't appear to be the same contact that's assigned to this order.");
                            return;
                        }
                        txtBillingAdr.Value = TheVendRMA.ContactVar.RefGet(x).BuildAddress();
                    }
                    ...
                    {
                        companyaddress a = companyaddress.GetByDescription(x, TheVendRMA.base_company_uid, val);
                        if (a != null)
            ...
            switch (val.Trim().ToLower())
            {
                ...
                default:
                    if (val.StartsWith("Contact address for"))
                    {
                        ...
                        txtShippingAdr.Value = TheVendRMA.ContactVar.RefGet(x).BuildAddress();
                    }
                    else if (val.ToLower().StartsWith("address option"))
                    {
        ...
        protected override void SaveData(Context x, SpotActArgs args, Dictionary<string, string> values)
        {
            ...
            if (!Tools.Strings.StrExt(s))
                return;
            foreach (orddet_line l in TheVendRMA.DetailsList((ContextRz)x))
            {
                if (!Tools.Strings.StrExt(l.shipvia_vendrma))
                {
                    l.shipvia_vendrma = s;
                    l.Update(x);
                }
        ...
        private void xCompany_CompanyChanged(ContextNM x, company c, ViewHandle v)
        {
            if (c == null)
                return;
            TheVendRMA.CompanyVar.RefSet(x, c);
            TheVendRMA.AbsorbCompany((ContextRz)x, c);
            TheVendRMA.Update(x);
            LoadAccountList((ContextRz)x);
            cboTerms.ValueSet(TheVendRMA.terms);
        ...
        private void xCompany_ContactChanged(ContextNM x, companycontact c, ViewHandle v)
        {
            ...
            txtFax.Change();
            txtEmail.Change();
            TheVendRMA.ContactVar.RefSet(x, c);
            TheVendRMA.Update(x);
        }
        private void lvDetails_AddNewItem(Context x, System.Web.UI.Page page, ViewHandle viewHandle)
        {
            TheVendRMA.DetailAddWithChecks((ContextRz)x);
            TheVendRMA.Update((ContextRz)x);
        ...
        private void lvDetails_ItemDoubleClicked(Context x, IItem item, System.Web.UI.Page page, ViewHandle viewHandle)
        {
            ...
            if (d == null)
                return;
            RzWeb.VendRMALine q = new RzWeb.VendRMALine((ContextRz)x, d);
