...
namespace NewMethodx
{
    ...
    public partial class nData : n_data_target
    {
        ...
        //    : base((Int32)t.target_type, t.server_name, t.database_name, t.user_name, t.user_password)
        //{
        //    SetConnectionString();
        //}

        //public nData(Int32 xType, String strServer, String strDatabase, String strUser, String strPassword)
        //    : base((Int32)xType, strServer, strDatabase, strUser, strPassword)
        //{
        //    base.SetConnectionInfo(xType, strServer, strDatabase, strUser, strPassword, "");
        //    SetConnectionString();
        //}
        ...
        //    : base((Int32)xType, strServer, strDatabase, strUser, strPassword, strAbsoluteConnectionString)
        //{
        //    base.SetConnectionInfo(xType, strServer, strDatabase, strUser, strPassword, strAbsoluteConnectionString);
        //    SetConnectionString();
        //}
        ...
        private System.Data.OleDb.OleDbConnection GetConnect(bool failok, ref String s)
        {
            ...
            catch (Exception e)
            {
                ...
                if (!failok)
                {
                    //if (!nTools.HasString(e.Message, "thread was being aborted"))
                    //    nStatus.TellUser(e.Message);
                }
        ...
        public void ExecuteAsyncHandler(Object a)
        {
            ...
            if (xConnect.State == ConnectionState.Open)
            {
                ...
                oCmd.CommandTimeout = nData.TimeOut;
                ...
                catch
                {
                    ...
                    {
                        //if (!nStatus.AskUser_YesNo("An error occured while " + args.strDescription + " [" + ex.Message + "].  Do you want to continue seeing these messages?"))
                        //{
        ...
        {
            if (target_type == (Int32)Enums.ServerTypes.MySQL)
                    ...
                    catch (Exception e)
                    {
                        ...
                        if (!failok)
                        {
                            if (!hidemessage)
                            {
                                ...
                                { }
                                //nStatus.TellUser("SQL Error: " + e.Message + "\r\n\r\n\r\n" + strSQL);
                            }
        ...
        public DataTable GetDataTable(String strSQL, bool FailOK, bool hidemessage, ref String strError)
        {
            ...
            //if (nData.SimulateDelay)
            //    System.Threading.Thread.Sleep(nTools.GetRandomInteger(3000, 8000));
            ...
            try
            {
                ...
                cmd.CommandTimeout = nData.TimeOut;
            ...
            catch (Exception e)
            {
                ...
                if (!FailOK)
                {
                    ...
                    if (!hidemessage)
                    {
                        //if (!nTools.StrCmp(e.Message, "thread was being aborted."))
                        //nStatus.TellUser(String.Concat("SQL Failed: \r\n\r\n", e.Message, "\r\n\r\n", strSQL));
                    }
                    //}
                }
                return null;
        ...
        //public ArrayList GetFieldArray_Prop(String strTable)
        //{
        //    ArrayList a = new ArrayList();
        //    DataTable t = GetDataTable(String.Concat("select top 1 * from ", strTable));
        //    if (t == null)
        ...
        //    for (int i = 0; i < t.Columns.Count; i++)
        //    {
        //        p = new n_prop((n_sys)null);
        //        p.name = t.Columns[i].ColumnName;
        //        p.property_type = (Int32)ConvertColumnTypeToPropertyType(t.Columns[i]);
        //        p.property_length = t.Columns[i].MaxLength;
        //        a.Add(p);
        //    }
        //    return a;
        //}
        ...
        {
            DataTable t = GetDataTable(str, true);
            return nTools.DataTableExists(t);
        }
        ...
        public bool FieldExists(DataTable dt, String strField)//?? I added for assistance with CheckCreateField
        {
            try
            {
                if (!nTools.StrExt(strField))
                    return false;
                foreach (DataColumn dc in dt.Columns)
                {
                    if (nTools.StrCmp(dc.ColumnName, strField))
                        return true;
                }
                return false;
            }
        ...
        public Object GetScalar(String strSQL, DataType xType)
        {
            ...
            try
            {
                switch (xType)
                {
                    ...
                    case DataType.Date:
                        return GetScalar(strSQL, nTools.GetBlankDate());
                    default:
                        return "";
                }
            }
        ...
        //{
        //    ArrayList a = new ArrayList();
        //    DataTable t = this.GetDataTable("select name from sysobjects where type = 'U' order by name");
        //    if (t == null)
        //        return a;
        ...
        public Object ConvertFromString(String strIn, Int32 t)
        {
            switch (t)
            {
                ...
                case (Int32)DataType.Date:
                    ...
                    {
                        return (Object)nTools.ReplaceNull((Int32)DataType.Date);
                    }
            }
            return 0;
        }
        ...
        //public bool BackupTableList(ArrayList a, ref String strFile)
        //{
        //    String s = this.database_name + "_backup_" + DateTime.Now.Year.ToString() + "_" + DateTime.Now.Month.ToString() + "_" + DateTime.Now.Day.ToString() + "_" + DateTime.Now.Hour.ToString() + "_" + DateTime.Now.Minute.ToString() + "_" + DateTime.Now.Second.ToString();
        //    if (DatabaseExists(s))
        //    {
        //        //nStatus.TellUser("The database " + s + " already exists");
        //        return false;
        //    }

        //    //nStatus.SetStatus("Creating " + s + "...");
        //    if (!CreateDatabase(s))
        //        return false;

        //    foreach (String t in a)
        //    {
        //        //nStatus.SetStatus("Copying " + t + "...");
        //        String strSQL = "select * into " + s + ".dbo." + t + " from " + t;
        //        if (!Execute(strSQL))
        //            return false;
        //    }

        //    nData d = new nData(new n_data_target(this.target_type, this.server_name, s, this.user_name, this.user_password));
        //    if (!d.CanConnect())
        //    {
        //        nStatus.TellUser("Could not connect to " + s);
        //        return false;
        //    }

        //    return d.Backup(ref strFile);
        //}
        ...
        public bool Backup(ref String strFile, bool hidemessage, ref String s)
        {
            ...
            if (!nTools.HasFileName(strFile))
                strFile = strFile + this.database_name + "_" + nTools.GetNowPath() + ".bak";

            //if( !System.IO.Directory.Exists(System.IO.Path.GetDirectoryName(strFile)) )
            //{
            //    nStatus.TellUser("The folder " + System.IO.Path.GetDirectoryName(strFile) + " does not exist.");
            //    return false;
            //}

            //if( System.IO.File.Exists(strFile) )
            //{
            //    nStatus.TellUser("The file " + strFile + " already exists.");
            //    return false;
            //}

            String strSQL;
            strSQL = "EXEC sp_dropdevice '" + this.database_name + "_bak'";
            nStatus.SetStatus("Dropping backup device...");
            long l = 0;
            Execute(strSQL, ref l, true, false);

            strSQL = "EXEC sp_addumpdevice 'disk', '" + this.database_name + "_bak', '" + strFile + "'";
            nStatus.SetStatus("Creating backup device...");
            if (!Execute(strSQL, false, hidemessage, ref s))
                return false;

            strSQL = "BACKUP DATABASE " + this.database_name + " TO " + this.database_name + "_bak";
            nStatus.SetStatus("Running the backup...");
            if (!Execute(strSQL, false, hidemessage, ref s))
            {
                nStatus.SetStatus("The backup was not successful.");
                return false;
            }
            else
            {
                //if( System.IO.File.Exists(strFile))
                //{
                nStatus.SetStatus("Backup Complete.");
                return true;
                //}
                //else
                //{
                //    nStatus.SetStatus("The backup returned a success code, but the file does not exist.");
                //    return false;
                //}
            }
        }

        public bool Restore(String strBakFile, String strDataFileMoniker, String strLogFileMoniker, String strDataPath, String strDatabaseName, ref String message)
        {
            try
            {
                String strSQL = "RESTORE DATABASE " + strDatabaseName + " FROM DISK = '" + strBakFile + "' WITH MOVE '" + strDataFileMoniker + "' TO '" + nTools.ConditionFolderName(strDataPath) + strDataFileMoniker + ".mdf', MOVE '" + strLogFileMoniker + "' TO '" + nTools.ConditionFolderName(strDataPath) + strLogFileMoniker + ".ldf'";
                return Execute(strSQL, false, true, ref message);
            }
            catch(Exception ex)
            {
                message = ex.Message;
        ...
        public bool Attach(String strMDF, String strLDF, String strDatabaseName, ref String message)
        {
            try
            {
                String strSQL = "";
                if( Tools.Strings.StrExt(strLDF) )
                    strSQL = "EXEC sp_attach_db @dbname = N'" + strDatabaseName + "', @filename1 = N'" + strMDF + "', @filename2 = N'" + strLDF + "'";
                else
                    strSQL = "EXEC sp_attach_single_file_db @dbname = N'" + strDatabaseName + "', @physname = N'" + strMDF + "'";
        ...
        //public bool Copy(String strNewName, string strNewFolder)
        //{
        //    if (!IsValidName(strNewName))
        //    {
        //        nStatus.Error("The database name " + strNewName + " is not a valid database item name.");
        //        return false;
        //    }

        //    if (DatabaseExists(strNewName))
        //    {
        //        nStatus.Error("The database name " + strNewName + " is already in use.");
        //        return false;
        //    }

        //    String strFile = strNewName;
        //    if (!Backup(ref strFile))
        //    {
        //        nStatus.Error("The backup step of the copy operation failed.");
        //        return false;
        //    }

        //    if (!Restore(strFile, strNewName, strNewFolder, this.database_name))
        //    {
        //        nStatus.Error("The restore step of the copy operation failed.");
        //        return false;
        //    }
        ...
        //public bool DatabaseExists(String s)
        //{
        //    nData t = new nData(this.target_type, this.server_name, s, this.user_name, this.user_password);
        //    return t.CanConnect();
        //}
        ...
        {
            return (nTools.StrCmp(strIn, nTools.StripNonAlphaNumeric(strIn, true)));
        }
        ...
        public bool RenameField(String strTable, String strFieldStart, String strFieldEnd)
        {
            return Execute("EXEC sp_rename '" + strTable + ".[" + strFieldStart + "]', '" + strFieldEnd + "', 'COLUMN'");
        }
        ...
        public bool CheckNotifyConnect()
        {
            ...
            {
                //nStatus.TellUser("The connection to this data source could not be made: " + s);
                return false;
            }
        }
        ...
        {
            return nTools.StrExt(GetScalar_String("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE OBJECTPROPERTY(OBJECT_ID(TABLE_NAME), 'TableHasIdentity') = 1 AND TABLE_TYPE = 'BASE TABLE' and table_name = '" + strTable + "' "));
        }
        ...
        //{
        //    nStatus.SetStatus("Truncating the transaction log...");

        //    long l = GetLogFileSizeInK();
        //    nStatus.SetStatus("Original log size: " + nTools.LongFormat(l) + " K");

        //    String s = GetScalar_String("select name from sysfiles WHERE FILEID=2");
        //    if (!nTools.StrExt(s))
        //        return false;

        //    if (!Execute("backup log " + database_name + " with truncate_only"))
        //        return false;

        //    bool b = Execute("dbcc shrinkfile( " + s + ", 10 )");

        //    l = GetLogFileSizeInK();
        //    nStatus.SetStatus("Current log size: " + nTools.LongFormat(l) + " K");
        //    nStatus.SetStatus("Truncate complete.");
        ...
        public static bool CopyTable(String source, String dest, String SourceTable, String DestTable, String strFields, ref String status)
        {
            try
            {
                ...
                {
                    String[] fields = nTools.Split(strFields, ",");
                    ...
                    copy.BulkCopyTimeout = nData.TimeOut;
                    copy.DestinationTableName = DestTable;
                    //nStatus.SetStatus("Transferring bulk data...");
        ...
        public bool ImportDataTable(DataTable d, String strTable, bool silent, ref String strStatus, int[] lens)
        {
            using (SqlBulkCopy copy = new SqlBulkCopy(GetConnectionString(true)))
            {
                ...
                catch (Exception ex2)
                {
                    ...
                    {
                        //nStatus.TellUser("Error in ImportDataTable: " + ex2.Message + "\r\n\r\n\r\n" + sb.ToString());
                    }
        ...
        public bool ImportDelimitedFileToTable(String strFile, Char limit, ref String strTable)
        {
            try
            {
                ...
                strTable = "temp_" + nTools.GetNewID();
        ...
        {
            if (nTools.StrCmp(server_name + "_-_" + database_name, xd.server_name + "_-_" + xd.database_name))
                return true;
        ...
        public bool ExportDataTableToCsv(DataTable d, String strFile, ref long l, String headerstring)
        {
            ...
            {
                //nStatus.SetStatus("Error in ExportDataTableToCsv: " + ex.Message);
                l = 0;
                return false;
            }
        }
        ...
        {
            if (nTools.StrCmp(str, "String"))
                return (Int32)DataType.String;
            if (nTools.StrCmp(str, "Long"))
                return (Int32)DataType.Long;
            if (nTools.StrCmp(str, "Float"))
                return (Int32)DataType.Float;
            if (nTools.StrCmp(str, "Memo"))
                return (Int32)DataType.Memo;
            if (nTools.StrCmp(str, "Integer"))
                return (Int32)DataType.Integer;
            if (nTools.StrCmp(str, "Date"))
                return (Int32)DataType.Date;
            if (nTools.StrCmp(str, "Boolean"))
                return (Int32)DataType.Boolean;
            if (nTools.StrCmp(str, "Blob"))
        ...
        {
            if (nTools.StrCmp(str, "Unknown"))
                return (Int32)DataUse.Unknown;
            if (nTools.StrCmp(str, "Any"))
                return (Int32)DataUse.Any;
            if (nTools.StrCmp(str, "TableSplit"))
                return (Int32)DataUse.TableSplit;
            if (nTools.StrCmp(str, "Email"))
                return (Int32)DataUse.Email;
            if (nTools.StrCmp(str, "Phone"))
                return (Int32)DataUse.Phone;
            if (nTools.StrCmp(str, "Url"))
                return (Int32)DataUse.Url;
            if (nTools.StrCmp(str, "List"))
                return (Int32)DataUse.List;
            if (nTools.StrCmp(str, "PersonName"))
                return (Int32)DataUse.PersonName;
            if (nTools.StrCmp(str, "FirstName"))
                return (Int32)DataUse.FirstName;
            if (nTools.StrCmp(str, "LastName"))
        ...
        public static Object GetNullValue(Enums.DataType t)
        {
            try
            {
                switch (t)
                {
                    ...
                    case DataType.Date:
                        return (Object)Convert.ToDateTime(nTools.GetNullDate());
                }
            }
            catch (Exception)
            { }
