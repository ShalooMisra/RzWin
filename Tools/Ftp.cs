...
namespace Tools
{
    ...
    public partial class Ftp
    {
        ...
        public static bool GetFileFTP(FTP ftplib, String strName, String strLocal, FTPProgressHandler progress, FTPStatusHandler status)
        {
            ...
            {
                if (status != null)
                    status("GetFileFTP Error: " + ex.Message);
                return false;
            }
        ...
        public static bool GetFileFTPDotNet(String strURI, String strLocalFile, FTPProgressHandler progress, FTPStatusHandler status, String strUserName, String strPassword, long totalsize)
        {
            ...
            //KT 3-17-2016 m- added thus to help troubleshoot failed uploads.       
            catch (WebException e)
            {
                String status4 = ((FtpWebResponse)e.Response).StatusDescription;
            }
            catch (Exception ex)
            {
                b = false;
        ...
        public static long GetFileSize(String uri, String user, String password)
        {
            try
            {
                FtpWebRequest reqSize = (FtpWebRequest)FtpWebRequest.Create(new Uri(uri));
                ...
                //or does this need to actually read the response?
                long ret = respSize.ContentLength;
    ...
    public class FTP
    {
        ...
        //{
        //    return SendFile(strLocal, Path.GetFileName(strLocal), ref status);
        //}
        ...
        public static bool SendFile(String strServer, String strUser, String strPassword, String strLocal, String strName, FTPProgressHandler progress, FTPStatusHandler status, List<String> folders)
        {
            FTP ftplib = new FTP(strServer, strUser, strPassword);
            ...
            if (status != null)
                status("Connected to " + strServer);

            bool ret = SendFile(ftplib, strLocal, strName, progress, status, folders);
        ...
        public static bool DeleteFile(String strServer, String strUser, String strPassword, String fileToDelete)
        {
            FTP ftplib = new FTP(strServer, strUser, strPassword);
            ...
            try
            {
                ftplib.RemoveFile(fileToDelete);
                ftplib.Disconnect();
                return true;
            }
            catch
        ...
        public static bool SendFile(FTP ftplib, String strLocal, String strName, FTPProgressHandler progress, FTPStatusHandler status, List<String> folders)
        {
            try
            {
                FileInfo fi = new FileInfo(strLocal);
                if (fi.Length > (1024 * 1024 * 10))
                    ftplib.timeout = 120000;
            ...
            {
                if (status != null)
                    status("SendFile Error: " + ex.Message);
                return false;
            }
        ...
        public static bool GetFile(FTP ftplib, String strName, String strLocal, FTPProgressHandler progress, FTPStatusHandler status)
        {
            try
            {
                ftplib.OpenDownload(strName, strLocal);
            ...
            {
                if (status != null)
                    status("GetFile Error: " + ex.Message);
                return false;
            }
